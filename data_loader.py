...
class Moving_MNIST_Loader:
    ...
    def get_batch(self, set, batch_size):
        if set == "train":
            ...
            return batch
    def get_anomaly_test_set(self, batch_size):
        #TODO create subset
        self.create_anomaly()
        return []
    ...
    def create_anomaly(self):
        ...
        anomaly = np.copy(self.data[:, rand_sequence, ...])
        ...
        for i in range(14):
            for j in range(14):
                if frame[i][j]!=0 and frame[i][j+1]!=0 and frame[i+1][j+1]!=0 and frame[i+1][j]!=0:
                    frame[i][j]=0
                    frame[i][j+1]=0
                    frame[i][j+2]=0
                    frame[i+1][j]=0
